...
static int create_and_bind_tcp(
    const char *host,
    const char *port
) {
...
void evloop_init(
    ...
    loop->periodic_maxsize =
        EVLOOP_INITIAL_SIZE;

    loop->periodic_nr = 0;

    loop->periodic_tasks = malloc(
        EVLOOP_INITIAL_SIZE *
        sizeof(*loop->periodic_tasks)
    );
...
void evloop_add_periodic_task(
    ...
    ((unsigned long long)seconds * 1000000ULL) +
    (ns / 1000ULL);
